...
public class FlipCards {
  ...
  public void reloadTexture(int frontIndex, View frontView, int backIndex, View backView) {
    synchronized (this) {
      ...
      if (KataponcoeLog.ENABLE_DEBUG) {
    	  KataponcoeLog.d("reloadTexture: activeIndex %d, front %d, back %d, angle %.1f",
  ...
  public synchronized void draw(FlipRenderer renderer, GL10 gl) {
    ...
    switch (state) {
      ...
      break;
      default:
    	  KataponcoeLog.e("Invalid state: " + state);
        break;
    }
          ...
          if (accumulatedAngle >= 0) {
            if (anglePageIndex != frontCards.getIndex()) {
              if (anglePageIndex == frontCards.getIndex() - 1) { //moved to previous page
                ...
                throw new RuntimeException(KataponcoeLog.format(
  ...
  private void setState(int state) {
    if (this.state != state) {                        /*
			if (KataponcoeLog.ENABLE_DEBUG)
				KataponcoeLog.i("setState: from %d, to %d; angle %.1f", this.state, state, angle);
			*/
      this.state = state;
      animatedFrame = 0;
    }
