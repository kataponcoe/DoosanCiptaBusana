...
import kataponcoe.flip.horizontal.GrabIt;
...
public class FlipViewController extends AdapterView<Adapter> {
  ...
  private Handler handler = new Handler(new Handler.Callback() {
    ...
    public boolean handleMessage(Message msg) {
      if (msg.what == MSG_SURFACE_CREATED) {
        ...
        KataponcoeLog.w("Unknown msg.what: " + msg.what);
      }
      return false;
    }
  });
  ...
  public FlipViewController(Context context, AttributeSet attrs, int defStyle) {
    ...
    TypedArray
        a =
        context.getTheme().obtainStyledAttributes(attrs, DebugKataponcoe.styleable.FlipViewController, 0, 0);

    try {
      int value = a.getInteger(DebugKataponcoe.styleable.FlipViewController_orientation, VERTICAL);
      if (value == HORIZONTAL) {
        orientation = HORIZONTAL;
      }

      value = a.getInteger(DebugKataponcoe.styleable.FlipViewController_animationBitmapFormat, 0);
      if (value == 1) {
  ...
  protected void onLayout(boolean changed, int l, int t, int r, int b) {
    if (KataponcoeLog.ENABLE_DEBUG) {
      KataponcoeLog.d("onLayout: %d, %d, %d, %d; child %d", l, t, r, b, bufferedViews.size());
    }
  ...
  private void debugBufferedViews() {
    if (KataponcoeLog.ENABLE_DEBUG) {
      KataponcoeLog.d("bufferedViews: %s; buffer index %d, adapter index %d", bufferedViews, bufferIndex,
                 adapterIndex);
    }
  }
  ...
  void flippedToView(final int indexInAdapter, boolean isPost) {
    if (KataponcoeLog.ENABLE_DEBUG) {
      KataponcoeLog.d("flippedToView: %d, isPost %s", indexInAdapter, isPost);
    }
    ...
    if (indexInAdapter >= 0 && indexInAdapter < adapterDataCount) {
      ...
      if (indexInAdapter == adapterIndex + 1) { //forward one page
        ...
        KataponcoeLog.e("Should not happen: indexInAdapter %d, adapterIndex %d", indexInAdapter,
                   adapterIndex);
      }
    } else {
